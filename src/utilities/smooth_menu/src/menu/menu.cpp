...
int Menu_t::addItem(std::string tag, int x, int y, int width, int height,
                    void* userData) {
...
int Menu_t::addItemVertically(std::string tag, int width, int height, int x,
                              void* userData) {
    int y;
    if (_item_list.size() == 0) {
        y = 0;
    } else {
        y = _item_list[_item_list.size() - 1]->y +
            _item_list[_item_list.size() - 1]->height - this->y;
    }

    return addItem(tag, x, y, width, height, userData);
}

int Menu_t::addItemHorizontally(std::string tag, int width, int height, int y,
                                void* userData) {
    int x;
    if (_item_list.size() == 0) {
        x = 0;
    } else {
        x = _item_list[_item_list.size() - 1]->x +
            _item_list[_item_list.size() - 1]->width - this->x;
    }
...
void Menu_t::update(uint32_t currentTime) {
    ...
    /* Iterate item list */
    for (int i = 0; i < _item_list.size(); i++) {
        _anim_cntr.open.setAnim(_cfg.animPath_open, 0, _item_list[i]->x_target,
                                _cfg.animTime_open);
        _item_list[i]->x = _anim_cntr.open.getValue(_anim_cntr.currentTime);
        _anim_cntr.open.setAnim(_cfg.animPath_open, 0, _item_list[i]->y_target,
                                _cfg.animTime_open);
